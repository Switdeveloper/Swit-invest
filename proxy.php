...
header('Access-Control-Allow-Headers: Content-Type, Authorization, X-Provider');
...
try {
    ...
    if (!$provider) {
        sendJSON(['error' => "Provider '$providerName' not found or inactive"], 404);
    }

    $apiKey = $provider['api_key'];
    if (empty($apiKey)) {
        sendJSON(['error' => "API key for '$providerName' is not set."], 401);
    }
